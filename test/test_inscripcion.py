...
class InscripcionTestCase(unittest.TestCase):
    ...
    def test_crear_inscripcion(self):
        ins = self.__nuevaInscripcion()
        InscripcionService.crear(ins)
        self.assertIsNotNone(ins.id)
        encontrada = InscripcionService.buscar_por_id(ins.id)
    ...
    def test_buscar_todos_inscripciones(self):
        ...
        InscripcionService.crear(ins1)
        InscripcionService.crear(ins2)
        todas = InscripcionService.buscar_todos()
    ...
    def test_actualizar_inscripcion(self):
        ...
        InscripcionService.crear(ins)
        ins.alumno_id = 42
        actualizado = InscripcionService.actualizar(ins)
        self.assertIsNotNone(actualizado)
        self.assertEqual(actualizado.alumno_id, 42)
        desde_db = InscripcionService.buscar_por_id(ins.id)
        self.assertEqual(desde_db.alumno_id, 42)

    def test_borrar_inscripcion(self):
        ins = self.__nuevaInscripcion()
        InscripcionService.crear(ins)
        borrado = InscripcionService.borrar_por_id(ins.id)
        self.assertIsNotNone(borrado)
        encontrado = InscripcionService.buscar_por_id(ins.id)
